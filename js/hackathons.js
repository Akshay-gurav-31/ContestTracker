...
function updateNextEvent() {
  ...
  nextEventEl.innerHTML = `
    ...
    <div class="event-card featured relative z-20">
      <div class="event-type-badge ${nextEvent.type}">${capitalizeFirstLetter(nextEvent.type)}</div>
      <h3 style="color: white !important;">${nextEvent.name}</h3>
      ...
      <div class="event-actions">
        ${nextEvent.url ? `<a href="${nextEvent.url}" target="_blank" class="event-url relative z-30">Visit Event Page</a>` : ''}
        <button class="delete-event-btn relative z-30" onclick="window.deleteHackathon('${nextEvent.id}')">Delete Event</button>
      </div>
    </div>
  `;
  nextEventEl.className = 'relative overflow-hidden rounded-2xl bg-slate-900 p-6 md:p-10 text-white shadow-2xl fade-in';
}
...
function renderHackathons(events) {
  ...
  hackathonsList.innerHTML = events
    .map(event => `
      <div class="group relative flex flex-col justify-between overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl">
        <div class="space-y-4">
          <div class="flex items-start justify-between gap-4">
            ...
            <div class="flex gap-2">
              ${event.url ? `<a href="${event.url}" target="_blank" class="text-slate-400 hover:text-primary transition-colors" title="Visit Link"><span class="material-symbols-outlined text-[18px]">open_in_new</span></a>` : ''}
              <button onclick="window.deleteHackathon('${event.id}')" class="text-slate-400 hover:text-red-500 transition-colors" title="Delete"><span class="material-symbols-outlined text-[18px]">delete</span></button>
            </div>
          </div>
          <div>
          ...
          ${event.description ? `<p class="text-sm text-slate-500 dark:text-slate-400 line-clamp-2 leading-relaxed">${event.description}</p>` : ''}
        </div>
      </div>
    `)
    .join('');
