/**
 * Generate fallback LeetCode data
 * @returns {Array} Array of contest objects
 */
export function generateLeetCodeFallback() {
  const contests = [];
  const today = new Date();
  for (let i = 0; i < 30; i++) {
    const date = new Date(today);
    date.setDate(today.getDate() + i);
    if (date.getDay() === 0) contests.push(createLeetCodeWeeklyContest(date));
    if (date.getDay() === 6) {
      const weekNumber = getWeekNumber(date);
      if (weekNumber % 2 != 0) contests.push(createLeetCodeBiweeklyContest(date));
    }
  }
  return contests;
}
...
/**
 * Generate contest data for platforms with fixed schedules (CodeChef, GFG)
 * @returns {Array} Array of contest objects
 */
...
function createLeetCodeWeeklyContest(date) {
  ...
  const contestNumber = 431 + Math.floor((date - new Date(2025, 0, 5)) / (7 * 24 * 60 * 60 * 1000));

  return {
    id: `lc-weekly-${contestNumber}`,
    platform: 'leetcode',
    name: `Weekly Contest ${contestNumber}`,
    url: `https://leetcode.com/contest/weekly-contest-${contestNumber}/`,
...
function createLeetCodeBiweeklyContest(date) {
  ...
  const biweeklyNumber = 121 + Math.floor((date - new Date(2025, 0, 4)) / (14 * 24 * 60 * 60 * 1000));
...
function createCodeChefContest(date) {
  ...
  if (dayOfMonth <= 7) {
    ...
    contestName = `Starters ${startersNumber} - ${getMonthName(date)} ${date.getFullYear()}`;
    contestUrl = `https://www.codechef.com/START${startersNumber}`;
  } else {
    contestName = `Starters ${startersNumber} (Lunchtime) - ${getMonthName(date)} ${date.getFullYear()}`;
    contestUrl = `https://www.codechef.com/START${startersNumber}`;
  }
...
function createGFGContest(date) {
  const contestDate = new Date(date);
  const gfgNumber = 84 + Math.floor((date - new Date(2025, 0, 5)) / (7 * 24 * 60 * 60 * 1000));

  return {
    id: `gfg-${formatDateSimple(date)}`,
    platform: 'gfg',
    name: `Weekly Coding Contest ${gfgNumber} - ${getMonthName(date)} ${date.getDate()}`,
    url: `https://practice.geeksforgeeks.org/contest/gfg-weekly-coding-contest-${gfgNumber}`,
    startTime: new Date(contestDate),
    duration: 2 * 60 * 60 * 1000, // 2 hours
    endTime: new Date(contestDate.getTime() + (2 * 60 * 60 * 1000)),
    durationFormatted: '2h'
